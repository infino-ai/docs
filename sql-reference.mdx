---
title: SQL reference for Infino queries and search functions
icon: "database"
description: Query Infino with SQL via query_sql, covering the supported dialect, the bm25_search, vector_search, and hybrid_search table functions, and how to compose them.
---
...
- **`token_match` / `exact_match`** are unranked: `score` is present (for schema
  ...
  Both read the full-text index, so `column` must be one you declared as an FTS
  column; naming an unindexed column fails with `unknown FTS column`. To match on
  a column without an index, filter it with plain SQL (`WHERE doc_id = '42'`).
