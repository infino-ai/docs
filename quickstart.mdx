...
<Accordion title="Node: the examples are ESM">
The Node examples below use `import`, so the file has to be a module. `npm init -y`
...
your `package.json` or save the file as `.mjs`.
</Accordion>
...
[Infino Cloud quickstart](/cloud/quickstart). Everything after the connect line is the
same either way.
...
use infino::{connect, Bm25SearchOptions, BoolMode, IndexSpec, Metric};
...
<CardGroup cols={2}>
  ...
  <Card title="Infino Cloud" icon="cloud" href="/cloud/quickstart">
