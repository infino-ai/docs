...
description: How object-storage-native retrieval works. Infino searches Apache Parquet files directly in S3 or Azure Blob, reading them in place.
icon: "cloud"
---
...
- **One copy, open format.** Because the files are a standard format, the same
  bytes that serve search also serve analytics, with no second system to sync.
...
high *first-byte latency*, so you cannot treat it like a local disk. Engines that
do this well share a few moves:
...
- **The superfile** is a single file that is *also a valid Apache Parquet file*:
...
- **The supertable** is many superfiles composed into one table with an
...
# queries directly against data on object storage, reading the files in place.
