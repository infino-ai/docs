---
title: "Infino tradeoffs: where it fits and where it doesn't"
...
- **Multi-modal retrieval over one copy.** BM25 full-text, vector search, and
  ...
  so there's no second system to sync and no client-side result stitching.
...
- **Light operational surface.** It runs in-process as a library, so there's no
  separate cluster to provision or keep warm.
