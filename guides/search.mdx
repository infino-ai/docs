...
| Hybrid | ✓ (RRF) | ✓ | n/a | ✓ (`hybrid_search`; also a direct method in Python and Node.js) |
...
semantic intent. Results come back best-first with a fused `score` (higher is better).
The table needs both indexes: an `fts` index on the text column and a `vector` index on
the embedding column.

<CodeGroup>

```python Python icon="python"
hits = docs.hybrid_search("body", "cancel subscription",
                          "embedding", embed("cancel subscription"), 5)
# tune the BM25 side with mode, the vector side with nprobe
hits = docs.hybrid_search("body", "cancel subscription",
                          "embedding", embed("..."), 5,
                          mode="and", nprobe=32)
```
...
```typescript Node.js icon="node-js"
const hits = docs.hybridSearch("body", "cancel subscription",
  "embedding", embed("cancel subscription"), 5);
// tune the BM25 side with mode, the vector side with nprobe
const more = docs.hybridSearch("body", "cancel subscription",
  "embedding", embed("..."), 5, { mode: "and", nprobe: 32 });
```

```rust Rust icon="rust"
// Rust exposes hybrid search through SQL. Pass the query vector as a
// comma-separated literal to the `hybrid_search` table function.
...
| Argument | Type | Default | Description |
| --- | --- | --- | --- |
| `text_column` | string | required | the FTS-indexed text column (BM25 side) |
| `text_query` | string | required | query terms, tokenized by the FTS index |
| `vector_column` | string | required | the vector-indexed column (kNN side) |
| `vector_query` | float[] | required | the query vector (same dim as the index) |
| `k` | int | required | number of top results (bounds each retriever and the fused result) |
| `mode` | `or` / `and` | `or` | BM25 boolean mode — match any term or require all terms |
| `nprobe` | int | engine default | IVF partitions to probe on the vector side; higher = better recall |
| `projection` | string[] | `_id` + `score` | columns to return |

Projection semantics match the other direct search methods: pass `projection=["doc_id",
"body"]` (or the Node.js equivalent) to pick which columns come back, and `score` is
always included.

### Compose in SQL

Hybrid search is also a SQL table function, so you can join or filter its results in a
query. Pass the query vector as a comma-separated literal:
...
The SQL form takes no `mode` or `nprobe` — use the direct `hybrid_search` method when you
need those. Build the vector literal from your own embedding, and escape any
user-supplied query text before putting it in SQL. See the [SQL Reference](/sql-reference)
for the full `hybrid_search` signature.
