...
| `rerankMult` *(Node)* | int | engine default | over-fetch multiplier for the exact-rerank stage |
...
(rows whose column equals a value) return every matching row, unranked — useful for
exact-id lookups and filtering.
