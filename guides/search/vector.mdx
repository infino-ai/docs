...
description: Run semantic kNN vector search in Infino with IVF indexes, tune recall and latency with nprobe, and narrow results with a pushdown text pre-filter.
---
...
- **Approximate (IVF).** Vector search trades exactness for speed; raise `nprobe` (and `rerank_mult`) to recover recall at some cost in work.
- **Bring your own embeddings.** Embed queries with the same model and dimension you indexed with — Infino does not embed for you.
- **The pushdown filter is a text predicate** over an FTS-indexed column. For scalar filters (ranges, equality on other columns), use [SQL](/sql-reference).
