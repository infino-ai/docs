...
tag: "Coming soon"
icon: "cloud"
description: Run the same Infino retrieval API against Infino Enterprise, the hosted service. Only the connect target changes; every table and search call stays the same.
---
...
Infino Enterprise is the same Infino API served as a hosted service. You don't run or size
...
You need two things from your dashboard:

1. **An API key** — a string beginning `ik_…`. Treat it like a password. See
   [Authentication](/guides/authentication).
2. **Your endpoint URL**, in the form `https://api.platform.infino.ai/<database>`. The scheme
   and host identify the service; the last path segment is the **database** this
   connection targets (for example `https://api.platform.infino.ai/my-app`). One connection is
...
pip install infino          # 0.1.10
```

```bash Node.js icon="node-js"
npm install @infino-ai/infino   # 0.1.9
```

```toml Rust icon="rust"
# Cargo.toml — 0.1.10, with the hosted transport enabled
[dependencies]
...
`connect("https://api.platform.infino.ai/my-app")` picks it up.
...
dashboard. On a local connection the same call is a harmless no-op, so setup code
written for Infino Enterprise also runs against `./data` or `s3://…` unchanged.
...
<Note>
On Infino Enterprise, `vector_search` and `hybrid_search` require an explicit `projection`
...
against `connect("./data")` moves to Infino Enterprise by changing only the `connect` line:
...
<Note>
Table maintenance (`optimize` and `gc`) is handled for you on Infino Enterprise, so you
never call it from the client.
</Note>

## See also

- [Authentication](/guides/authentication) — API keys, the `Authorization` header, and rotation.
