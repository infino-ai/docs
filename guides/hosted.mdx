...
2. **Your endpoint URL**, in the form `https://<your-endpoint>/<database>`. The scheme
   and host identify the service; the last path segment is the **database** this
   connection targets (for example `https://<your-endpoint>/my-app`). One connection is
...
db = infino.connect("https://<your-endpoint>/my-app", api_key="ik_…")
```

```typescript Node.js icon="node-js"
import { connect } from "@infino-ai/infino";

const db = connect("https://<your-endpoint>/my-app", { apiKey: "ik_…" });
```
...
`connect("https://<your-endpoint>/my-app")` picks it up.
