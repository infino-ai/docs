...
row-by-row for throughput. On Infino Cloud, one append request is capped at 128 MiB of
Arrow data; target 100 MiB or less per batch (see [Limits](/cloud/limits)).
</Note>
...
db.list_tables()                   # ["docs"]
db.drop_table("docs")              # drop + reclaim storage (purge defaults to True)
db.drop_table("docs", purge=False) # …or unregister only, leaving the bytes
```

```typescript Node.js icon="node-js"
docs.optimize();                   // merge small / underfilled superfiles
db.listTables();                   // ["docs"]
db.dropTable("docs");              // drop + reclaim storage (purge defaults to true)
db.dropTable("docs", false);       // …or unregister only, leaving the bytes
```
...
db.drop_table("docs", true)?;                 // true reclaims storage; false unregisters only
```
...
| `stale_seal_timeout_ms` / `staleSealTimeoutMs` | how old a sealed tombstone sidecar must be, in ms, before compaction treats its owner as dead |
| `recalibrate` | vector probe-law recalibration: `auto` (default), `force`, or `skip` |
